...
import {MemoryCryptoStore} from 'matrix-js-sdk/lib/crypto/store/memory-crypto-store.js'
...
async function main() {
    ...
    const client = sdk.createClient({
        ...
        userId: '@austinkregel:communication.ventures',
        deviceId: process.env.MATRIX_DEVICE,
    });

    try {
        console.log('client',await client.whoami())
    } catch (e) {
        console.log('found client error');
        console.error(e)

        process.exit(0)
    }

    console.log('Initializing crypto client')

    await client.initCrypto();
    console.log('Initialized crypto client')
    ...
    //         console.log('Room encrypted event', roomId, event)
