...
//        $this->removeImport([$listener]);
    ...
    protected function isClassInstanceOfModel(string $listenerParameterType)
    {
        if (empty($listenerParameterType)) {
            return false;
        }

        if (!class_exists($listenerParameterType)) {
            return false;
        }
        $models = static::instancesOf(Model::class)->getClasses();

        if (!in_array($listenerParameterType, $models)) {
            return false;
        }
        // it's a laravel model
        return true;
    }

    protected function recurseGetUnionType(\ReflectionNamedType | \ReflectionUnionType $type): array
    {
        ...
        return array_reduce($types, function ($allBits, \ReflectionNamedType | \ReflectionUnionType $type) {
            return array_merge($allBits,[
                method_exists($type, 'getTypes') ? $type->getTypes() : $type->getName()
            ]);
        }, []);
    }
    ...
    public static function findLogicalEvents(): array
    {
        ...
        $nonExampleEvents = array_filter($code->getClasses(), fn ($class) => !str_contains($class, 'Example'));

        return array_reduce($nonExampleEvents, function ($allClasses, $instanceOfLogicalEvent) use ($code){
            $actualListeners = [];
            ...
            // Only technically exist for class types and not interfaces
            $constructorParametersThatAreModels = $logicalEventReflection->getConstructor()?->getParameters() ?? [];
            ...
            $methodsActuallyDefinedOnOurLogicalEvent = array_filter(array_diff($methodNamesOnClass, $methodsProvidedByTraits), fn ($value) => $value !== "__construct");
            ...
            return array_merge($allClasses, [
                $instanceOfLogicalEvent => [
                    ...
                    'methods' => array_reduce($methodsActuallyDefinedOnOurLogicalEvent, function ($allMethods, $method) use ($logicalEventReflection, $code, $instanceOfLogicalEvent, $netteCodeInstance) {
                        try {
                        ...
                        return array_merge($allMethods, [
                            $method => [
                                'parameters' => array_map(function (PromotedParameter $param) use ($code) {;
