...
use App\Events\Admin\Composer\ActionFailed;
use App\Events\Admin\Composer\ActionFinished;
use App\Events\Admin\Composer\ActionLoggedToConsole;
...
class DisableProviderController extends Controller
{
    public function __invoke(Filesystem $filesystem)
    {
        ...
        $queuedInstalledProcess = function () use ($jobId, $filesystem, $name) {
            ...
            broadcast(new ActionLoggedToConsole($jobId, "Attempting to identify the driver needed\r\n"));
            ...
            broadcast(new ActionLoggedToConsole($jobId, "\e[01;32mThere are {$configuredServices->count()} possible oauth/socialite services.\r\n"));
                ...
                foreach ($drivers as $class => $driverName) {
                    try {
                        if (!$configuredServices->has($driverName)) {
                            broadcast(new ActionLoggedToConsole($jobId, "\e[01;31mThere are no services configured for driver [" . $driverName . "]. Please update your config/services.php config file.\r\n"));
                            continue;
                        }

                        $code->removeListenerFromEvent(SocialiteWasCalled::class, $class);

                        broadcast(new ActionLoggedToConsole($jobId, "\e[01;32mLooks like we are able to disable the event listener automatically.\r\n"
                            ."Attempting to save file...\r\n"));

                        $code->toFile(Code::REPLACE_FILE);
                        broadcast(new ActionLoggedToConsole($jobId, "\e[01;33mFile: app/Providers/EventServiceProvider.php updated successfully.\r\n"));
                    } catch (\Throwable $e) {
                        broadcast(new ActionLoggedToConsole($jobId, $e->getMessage()));
            ...
            broadcast(new ActionLoggedToConsole($jobId, "\e[01;33mDisabling complete, you can close this window.\r\n"));
        };
