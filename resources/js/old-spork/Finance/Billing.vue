<template>
    <div class="flex flex-wrap mt-4">
        <div class="w-full py-2 px-4 text-2xl font-bold text-stone-800 dark:text-stone-200 flex items-center justify-between">
            <span>Budgets Dashboard</span>
            <span>
                <feature-required feature="budgets" allow-more-than-one="true" />
            </span>
        </div>
        <div class="w-full pb-4 px-4 text-base font-base text-stone-500 dark:text-stone-300">Greetings! It's 2021-01-01. You should expect $420.20 to be withdrawn today.</div>
        <!-- The budgeting information -->
        <div class="w-full flex flex-wrap py-4 m-4 bg-white dark:bg-stone-600 rounded-lg shadow">
            <div class="w-full flex mx-4 rounded-full overflow-none bg-stone-200 my-2 text-white font-bold items-center">
        ...
        <div class="w-full px-4">
            <h3 class="text-lg leading-6 font-medium text-stone-900 dark:text-stone-50">
                Month to date
            </h3>

            <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
                <div v-for="item in stats" :key="item.name" class="px-4 py-5 bg-white dark:bg-stone-600 shadow rounded-lg overflow-hidden sm:p-6">
                    <dt class="text-sm font-medium text-stone-500 dark:text-stone-300 truncate">
                    {{ item.name }}
                    </dt>
                    <dd class="mt-1 text-3xl font-semibold text-stone-900 dark:text-stone-100">
                    {{ item.stat }}
                    </dd>
                </div>
            </dl>
        ...
        <div class="w-1/3">
            ...
            <div class="bg-white dark:bg-stone-600 shadow overflow-hidden sm:rounded-md m-4 px-4 py-2 flex flex-col max-h-4xl divide-y divide-stone-200 items-center">
                <div class="flex w-full items-center gap-2" v-for="transaction in paidBudgets" :key="transaction">
                    ...
                    <div class="flex flex-wrap w-full items-center py-2">
                        <div class="w-full flex justify-between items-center">
                            ...
                            <div class="text-right text-stone-700 dark:text-stone-200 font-bold">${{ transaction.amount}}</div>
                        </div>

                        <div class="w-full flex justify-between items-center text-sm text-stone-600 dark:text-stone-300">
                            <div class="text-stone-500 dark:text-stone-300">{{ transaction.category }}</div>
                ...
                <div v-if="paidBudgets.length === 0">
                    <div class="text-center text-stone-500 dark:text-stone-300">No paid budgets</div>
                </div>
            </div>
        </div>
        <div class="w-1/3">
            ...
            <div class="bg-white dark:bg-stone-600 shadow overflow-hidden sm:rounded-md m-4 px-4 py-2 flex flex-col max-h-4xl divide-y divide-stone-200 items-center">
                ...
                <div v-if="pendingBudgets.length === 0">
                    <div class="text-center text-stone-500 dark:text-stone-300">No pending budgets</div>
                </div>
            </div>
        </div>
        <div class="w-1/3">
            ...
            <div class="bg-white dark:bg-stone-600 shadow overflow-hidden sm:rounded-md m-4 px-4 py-2 flex flex-col max-h-4xl divide-y divide-stone-200 items-center">
               <div class="flex w-full items-center gap-2" v-for="transaction in futureBudgets" :key="transaction">
                    ...
                    <div class="flex flex-wrap w-full items-center py-2">
                        <div class="w-full flex justify-between items-center">
                            <div class="font-medium">{{ transaction.name }}</div>
                            <div class="text-right text-stone-700 dark:text-stone-200 font-bold">${{ transaction.settings.amount}}</div>
                        </div>

                        <div class="w-full flex justify-between items-center text-sm text-stone-600 dark:text-stone-300">
                            <div class="text-stone-500 dark:text-stone-300">{{ transaction.tags?.map(tag => tag?.en ?? '')?.join(', ') }}</div>
                            <div class="text-right">{{ transaction.repeatable }}</div>
                        </div>
                    </div>
                </div>
                <div v-if="futureBudgets.length === 0">
                    <div class="text-center text-stone-500 dark:text-stone-300">No future budgets</div>
                </div>
            </div>
        </div>
    </div>
