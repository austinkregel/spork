<template>
    <div class="m-4">
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1">
                <div class="px-4 sm:px-0">
                    <h3 class="text-lg font-medium leading-6 text-stone-900 dark:text-stone-50">Accounts</h3>
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <div v-for="(tokens, $i) in [$store.getters.manualFinance].filter(v => v)" :key="$i" class="mx-4">
                    <div class="w-full text-stone-600 dark:text-stone-300 uppercase">{{ tokens.name }}</div>
                    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" v-if="tokens?.accounts?.length > 0">
                        <div v-for="item in tokens?.accounts" :key="item.id" class="relative pt-5 px-4 pb-12 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden bg-white dark:bg-stone-600">
                            <dt>
                                <div class="absolute bg-blue-500 rounded-md p-3">
                                    <office-building-icon class="h-6 w-6 text-white" aria-hidden="true" />
                                </div>
                                <p class="ml-16 text-sm font-medium text-stone-500 truncate">{{ item.name }}</p>
                            </dt>
                            <dd class="ml-16 pb-6 flex items-baseline sm:pb-7">
                                <p class="text-2xl font-semibold text-stone-900">
                                    {{ item.balance }}
                                </p>
                                <div class="absolute bottom-0 inset-x-0 bg-stone-5000 px-4 py-4 sm:px-6 flex justify-between items-center">
                                    <div class="text-sm">
                                    ...
                                    <div>
                                        <button @click.prevent="() => deleteAccount(item)">
                                            <trash-icon class="h-6 w-6 text-stone-400 hover:text-red-500" aria-hidden="true" />
                                        </button>
                                    </div>
                                </div>
                            </dd>
                    ...
                    <div v-else class="p-4 bg-white dark:bg-stone-600 italic text-stone-600 dark:text-stone-300 dark:text-stone-200 shadow rounded mt-2">
                        No accounts found.
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="my-4">
                        <div class="text-left">
                            <label class="block text-sm font-medium text-stone-700 dark:text-stone-200" for="account_selector">
                                Finance Account (Link)
                            </label>
                            <div class="mt-2">
                                <select v-model="link_id" class="placeholder-stone-400 block max-w-lg w-full shadow-sm bg-white dark:bg-stone-500 dark:border-stone-500 focus:ring-blue-500 focus:border-blue-500 sm:text-sm border-stone-300 rounded-md">
        ...
        <div class="hidden sm:block" aria-hidden="true">
            <div class="py-5">
                <div class="border-t border-stone-200 dark:border-stone-600" />
            </div>
        </div>
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1">
                <div class="px-4 sm:px-0">
                    <h3 class="text-lg font-medium leading-6 text-stone-900 dark:text-stone-50">Transactions</h3>
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                ...
                >
                    <div class="mt-4">
                        ...
                        <select v-model="account_id" id="account_selector" class="placeholder-stone-400 mt-1 block w-full py-2 px-3 border border-stone-300 dark:border-stone-500 bg-white dark:bg-stone-500 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
