...
::-webkit-scrollbar {
    width: 8px;
    height: 8px;
    @apply scrollbar-thumb-slate-600 scrollbar-track-slate-700;
}
...
<template>
    <div v-if="hasOpenFiles" class="overflow-hidden bg-slate-800 h-full flex-1 flex flex-col justify-between pt-1">
        <div class="py-4 px-2 flex justify-between">
            <div>
                <div class="flex flex-col">
                    <div class="text-lg">In {{ $store.getters.openProject?.name }}</div>
                    <pre class="text-xs text-slate-400">{{ $store.getters.openProject?.settings?.path?.replace('/var/www/html', '') }}</pre>
                </div>
                <div class="flex flex-wrap items-center gap-4 text-xs mt-4">
                    <button class="flex items-center gap-1" :disabled="$store.getters.tab.isDirty" :class="[$store.getters.tab.isDirty ? '' : 'opacity-75 cursor-not-allowed']">
                        <CheckIcon class="w-4 h-4 fill-current text-slate-200"/>
                        Save
                    </button>
                    <button>
            ...
            <div>
                <div class="relative inline-block text-left">
                    <div>
                        <button type="button" class="inline-flex justify-center w-full rounded-md border border-slate-300  dark:bg-slate-600 dark:text-white shadow-sm px-4 py-2 bg-white text-sm font-medium text-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:border-slate-600 focus:ring-offset-slate-100 focus:ring-indigo-500" id="menu-button" aria-expanded="true" aria-haspopup="true"
                    ...
                    <div v-if="editorSettings" class="origin-top-right z-10 absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white dark:bg-slate-700 ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                        <div class="p-2 space-y-2" role="none">
                            <select v-model="selectedTheme" class="rounded bg-white dark:bg-slate-600 text-slate-800 dark:text-slate-200 placeholder-slate-200">
                                <option v-for="theme in themeList" :key="'theme.'+theme.name" :value="theme">{{ theme.name }}</option>
                            </select>
                            
                            <select v-model="selectedLanguage" class="rounded bg-white dark:bg-slate-600 text-slate-800 dark:text-slate-200 placeholder-slate-200">
        ...
        <div class="flex items-center border-b border-slate-700 px-1 max-w-full overflow-x-scroll">
            <button 
                v-for="file in openFiles" 
                :key="file.absolute"
                class="rounded-t-lg py-1 px-2 flex items-center text-sm tab z-0 mx-1" 
                :class="[
                    $store.getters.tab?.absolute === file.absolute ? 'bg-slate-600 active' : 'bg-slate-700',
                ]" 
