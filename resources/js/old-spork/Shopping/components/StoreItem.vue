<template>
    <div class="col-span-1 flex flex-col text-center bg-white rounded-lg shadow divide-y divide-stone-200">
        <div class="flex-1 flex flex-col p-8">
            <img class="w-32 h-32 flex-shrink-0 mx-auto rounded-full" :src="item.image_url" alt="" />
            <h3 class="mt-6 text-stone-900 text-sm font-medium">{{ item.name }}</h3>
        </div>
        <div>
            <div class="-mt-px flex divide-x divide-stone-200">
                <div class="w-0 flex-1 flex">
                    <button @click.prevent="() => $store.dispatch('addItemToCart', { id: item.id, price: item.price, name: item.name, image_url: item.image_url, count: item.count, store: item.store })"
                            class="gap-2 relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-stone-700 font-medium border border-transparent rounded-bl-lg hover:text-stone-500">
                        <ShoppingCartIcon class="w-5 h-5 text-stone-400" aria-hidden="true" />
                ...
                <div class="-ml-px w-0 flex-1 flex" v-if="itemsInCart[item.id]">
                    <button @click.prevent="() => $store.dispatch('removeItemToCart', item)"
                            class="gap-2 relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-stone-500 font-medium border border-transparent rounded-br-lg hover:text-red-600">
