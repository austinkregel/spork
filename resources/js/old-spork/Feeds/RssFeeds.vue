<template>
    <ul role="list" class="grid grid-cols-2 ">
        <li v-for="(event, eventIdx) in $store.getters.feeds" :key="eventIdx" class="w-full p-2 group">
            <div class="relative rounded shadow bg-white dark:bg-stone-600">
                <div class="absolute bottom-0 z-10 bg-stone-900 hidden group-hover:block h-0 group-hover:h-75 w-full bg-stone-100" v-if="event.attachment">
                ...
                <div class="px-4 pt-4 ">
                    <div class="font-bold text-lg mb-2">
                        ...
                        <span class="inline-block rounded-full font-semibold text-stone-700 dark:text-stone-300">
                            {{event.author?.name}}
                        </span>

                        <div class="text-xs text-stone-400 dark:text-stone-300 mt-2 ">{{ dayjs.utc(event.last_modified).from(dayjs()) }}</div>
                    </div>
