<template>
    <div class="w-full">
        <crud-view
            ...
            @index="() => ''"
            @execute="onExecute"
            :data="$store.getters.features?.favorite"
        ...
        >
            ...
            <template v-slot:modal-title>Add to your Favorite</template>
            ...
            <template #no-data>No favorite in your Favorite</template>
        </crud-view>
...
export default {
    ...
    methods: {
        ...
        dateFormat(contact) {
            return '<span class="text-gray-900">' + contact.starts_at  + '  at </span>' +
                '<span class="text-gray-800">' + dayjs(contact.last_occurrence || contact.remind_at).format('h:mma') + '</span>'
        },
        async save(form) {
            if (!form.id) {
                this.$store.dispatch('createFavorite', form)
            } else {
                console.log('No edit method defined')
            }
        },
        async onDelete(data) {
            await this.$store.dispatch('deleteFavorite', data);
