...
const submit = () => {
  form.post(route('automation.automations.store'));
};
</script>
  ...
  <Manage :title="title" sub-title="Automation" home="/-/automation">
    <form @submit.prevent="submit" class="space-y-6">
      <header class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900 dark:text-white">Create Automation</h1>
        <div class="flex items-center gap-2">
          <Link :href="route('automation.automations.index')" class="px-3 py-2 text-sm rounded-md border border-stone-300 dark:border-stone-700">Cancel</Link>
