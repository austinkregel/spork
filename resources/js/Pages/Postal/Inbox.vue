    ...
    <AppLayout title="Postal/Inbox">
        ...
        <main class="flex flex-wrap w-full h-full overflow-hidden justify text-white" style="max-height: calc(100vh - 65px);">
          <div class="relative flex flex-col rounded divide-stone-600 divide-y w-1/2 xl:w-1/4 overflow-y-scroll" style="max-height: calc(100vh - 65px);">
            ...
            <button
                @click="() => changeIframe(item)" v-for="item in page.props.messages?.data"
                :key="item"
          ...
          <div v-show="openContext && openForMail">
            ...
            <div class="absolute transition-all z-10 mt-2 w-56 overflow-hidden rounded-md shadow-lg bg-white dark:bg-stone-600 ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1"
...
const open_in_new_tab = () => {
    window.open(window.location.href+"/"+ openForMail.value.event_id, '_blank').focus();
...
const mark_as_unread = async () => {
  loading.value = true;
  await axios.post('/api/mail/mark-as-unread', {
    id: openForMail.value.event_id
  });
  await triggerMailLoading();
}
